...
{
    public class ChatService : GrpcChatService.ChatService.ChatServiceBase
	{
        private readonly ILogger<ChatService> _logger;
        private readonly IChatRoomService _chatRoomService;


        public ChatService(ILogger<ChatService> logger, IChatRoomService chatRoomService)
        {
            _logger = logger;
            _chatRoomService = chatRoomService;
        }

        public override async Task<JoinResponse> JoinToChatRoom(JoinRequest request, ServerCallContext context)
        {
            return new JoinResponse { RoomId = await _chatRoomService.AddCustomerToChatRoomAsync(request.Customer) };
        }

        public override async Task SendMessage(IAsyncStreamReader<ChatMessage> requestStream, IServerStreamWriter<ChatMessage> responseStream,
